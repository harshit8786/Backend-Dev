...
function deleteStudent(id){
    ...
    if(!(data && data.length>0)) throw new Error(" data is not exit ")
        let isid = data.some((valu)=> valu.id==id)
    if(!isid) throw new Error(" id  is  not exit")
    let newData = data.filter((value)=> value.id!=id)

    fs.writeFileSync("student.json", JSON.stringify(newData, null, 2))
 console.log(" student delete susscfull");
